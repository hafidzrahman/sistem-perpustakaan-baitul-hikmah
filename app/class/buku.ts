...
export class Buku {
  ...
  static async tambahBuku(
    dataBuku: tambahBukuType
  ): Promise<eksemplarDenganBukuType> {
    const {
      judul,
      isbn,
      linkGambar,
    ...
    if (penulis && typeof penulis[0] !== "number") {
      penulis = (await konversiDataKeId(
        "penulis",
        penulis as string[]
      )) as number[];
    }
    ...
    if (penerbit && typeof penerbit !== "number") {
      penerbit = (await konversiDataKeId(
        "penerbit",
        penerbit as string
      )) as number;
    }
  ...
  static async tambahBanyakBuku(
    dataBuku: (bukuType & eksemplarBukuType)[]
  ): Promise<void> {
  ...
  static async cariBuku(
    isbn: string
  ): Promise<detailsBukuType | undefined | null> {
  ...
  static async perbaruiBuku(
    isbn: string,
    dataBuku: perbaruiBukuType
  ): Promise<void> {
    ...
    console.log("Data sebelum konversi:", dataBuku);
    console.log("Data setelah konversi penulis:", penulis);
    console.log("Data setelah konversi penerbit:", penerbit);
    console.log("Data setelah konversi genre:", genre);

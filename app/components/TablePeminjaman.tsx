...
import ModalDetailPeminjaman from "./modal/ModalDetailPeminjaman";
...
const TablePeminjaman = ({
  ...
  const [selectedPeminjaman, setSelectedPeminjaman] =
    useState<peminjamanType | null>(null);
  ...
  return (
    <>
                      ...
                      {item.bukuPinjaman.map(
                        (buku, bukuIndex) =>
                          !buku.tanggalKembali && (
                            <button
                              key={bukuIndex}
                              onClick={() => setSelectedPeminjaman(item)}
      ...
      <ModalDetailPeminjaman
        ...
        bukuInfo={
          selectedPeminjaman
            ? getBukuData(selectedPeminjaman.bukuPinjaman?.[0]?.bukuISBN || "")
            : null
        }
        peminjam={
          selectedPeminjaman
            ? getPeminjamData(selectedPeminjaman.nis, selectedPeminjaman.nip)
            : null
        }
      />
    </>

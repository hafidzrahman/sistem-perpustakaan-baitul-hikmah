...
interface LineChartPeminjamanMuridProps {
  data: peminjamanType[];
  nis?: string;
}
...
const LineChartPeminjamanMurid = ({
  data,
  nis,
}: LineChartPeminjamanMuridProps) => {
  const processSemesterData = React.useMemo(() => {
    if (!data || !nis) return [];

    // Filter for current student's loans
    const studentLoans = data.filter((loan) => loan.nis === nis);
...
export default LineChartPeminjamanMurid;

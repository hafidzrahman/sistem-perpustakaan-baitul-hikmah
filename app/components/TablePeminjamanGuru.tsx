...
interface TablePeminjamanGuruProps {
  data: peminjamanType[];
  bukuList: bukuType[];
  nip?: string;
}

const TablePeminjamanGuru = ({
  data = [],
  bukuList = [],
  nip,
}: TablePeminjamanGuruProps) => {
  ...
  const filteredData = React.useMemo(() => {
    ...
    // Filter loans for current teacher and by search query
    return data.filter((item) => {
      // Using nipPeminjam instead of nip for filtering
      if (item.nip !== nip) return false;
...
export default TablePeminjamanGuru;

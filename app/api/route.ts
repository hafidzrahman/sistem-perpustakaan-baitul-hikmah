...
export async function GET() {
  ...
  let arrayKeterangan: keteranganType[] =
    (await keterangan.cariKeterangan()) as keteranganType[];
  let arrayGuru: guruType[] = (await guru.cariAnggota()) as guruType[];
  let arrayPeminjaman: peminjamanType[] =
    (await peminjaman.cariPeminjaman()) as peminjamanType[];

  return NextResponse.json({ arrayPeminjaman });
}
